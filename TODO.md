...
1. [ ] **Use confidence language that helps make a decision.** `Medium`, `Low` and `Learned` expose the recognition model and are all styled green. Prefer user-facing states such as `Needs checking`, `Suggested` and `Confirmed`, with orange reserved for required review and green for confirmed results; omit the badge when no action or useful assurance remains.
2. [ ] **Divide Activities by purpose.** Show recently used activities first, history-only labels awaiting adoption in their own actionable section, and collapse `Not used yet` labels by default. A search field becomes worthwhile once those sections still span several screens.
3. [ ] **Give date-picker sheets reversible behaviour.** Timeline and Insights currently write the selected date immediately but offer only `Done`. Either dismiss immediately after a deliberate date selection, or edit a draft and provide standard Cancel and Done actions so an accidental change can be abandoned.
4. [ ] **Stop Settings copy promising explicit Home and Work roles before they exist.** The current footer says locations can be set as Home or Work, while that meaning is still inferred from a place name. Use accurate generic saved-place wording until the explicit place-role item under Health and movement ships.
