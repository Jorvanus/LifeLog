...
- [ ] **Workout routes were never authorised.** They are a separate permission, added to the requested set after the others had been granted, and the old prompt gate meant iOS was never asked. Every walk recorded since then has no route, which is what `ActivityLocationPolicy.leftStay` needs to tell a loop around the block from pacing at home. Nothing backfills them: Health keeps the route, so a re-import of past workouts should recover them once access is granted. Worth checking whether the anchored importer will pick up routes for workouts it has already seen, or whether that needs an anchor reset.
