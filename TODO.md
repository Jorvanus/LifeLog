...
- [ ] **The 8 August Home/Gracemere overlap is unconfirmed after the walk-timing fix.** That day's Insights export showed a Home stay running 8:01–9:38 while the next stay (Gracemere Shopping World) arrived at 9:36 — a two-minute overlap, both ends stamped 09:38:01. That timestamp was `boundStay` pulling a departure forward to where a routeless walk's steps began, the same mechanism `maximumStayShareConsumedByJourney` now constrains (below) — but that guard is on the walk's own bounding, not on Home's departure directly, so whether *this* overlap is gone is a separate question from whether the walk was fixed. Pull a fresh Insights → Export CSV and check.
