...
- [ ] **Verify the new foreground-only burst on hardware.** It runs at launch/foreground and for an arriving `CLVisit`, but has deliberately not been wired into the long-running background workflow. No stationary evidence, an error or a timeout creates no visit; it must never fall back to the old single-fix inference. Device checklist: standing still indoors settles within fifteen seconds; walking continuously creates nothing; poor signal logs a clean failure rather than hanging.
...
### Visual review — 2026-08-09

Checked on the iPhone 17 Pro Max under iOS 27 at the normal text size in light
appearance and AccessibilityXXXL in dark appearance. Work through these in order:

1. [ ] **Simplify location permission setup in Settings.** Replace the competing `Permission`, `Background location logging` and `Allow while using` controls with one clear enable-location action. Reveal background logging only after When In Use permission succeeds, and explain when Always access is required.
2. [ ] **Separate Apple Health from Motion Activity setup.** A `Connect Apple Health` button cannot resolve Motion permission, yet both states currently sit in one section. Give each source its own state, recovery action and explanation.
3. [ ] **Make Insights highlights actionable and human-readable.** Suppress comparisons whose leading category is `Other`, or name the underlying unadopted activity. Change day subtitles such as `2026, Sunday` to a natural localized date, and move the Apple Health connection prompt out of the donut into a real button or setup row.
4. [ ] **Remove duplicate affordances.** The Timeline review card is simultaneously a full-card link, a `Check` pill and a chevron; retain one visible action cue. On Insights, decide from use whether both the tappable period title and separate calendar toolbar button earn their space.
5. [ ] **Use confidence language that helps make a decision.** `Medium`, `Low` and `Learned` expose the recognition model and are all styled green. Prefer user-facing states such as `Needs checking`, `Suggested` and `Confirmed`, with orange reserved for required review and green for confirmed results; omit the badge when no action or useful assurance remains.
6. [ ] **Divide Activities by purpose.** Show recently used activities first, history-only labels awaiting adoption in their own actionable section, and collapse `Not used yet` labels by default. A search field becomes worthwhile once those sections still span several screens.
7. [ ] **Give date-picker sheets reversible behaviour.** Timeline and Insights currently write the selected date immediately but offer only `Done`. Either dismiss immediately after a deliberate date selection, or edit a draft and provide standard Cancel and Done actions so an accidental change can be abandoned.
8. [ ] **Stop Settings copy promising explicit Home and Work roles before they exist.** The current footer says locations can be set as Home or Work, while that meaning is still inferred from a place name. Use accurate generic saved-place wording until the explicit place-role item under Health and movement ships.
9. [ ] **Build screenshot regression coverage for the iPhone 17 Pro Max.** Capture at least normal/light, normal/dark, AccessibilityXXXL/light and AccessibilityXXXL/dark for the four primary tabs. Include the review card, a crowded Activities list and the initial Settings permission state; frame assertions alone do not catch overlapping or fragmented text.
...
- [ ] Check the remaining secondary screens at AccessibilityXXXL in both appearances. Timeline, Insights, Activities and Settings have now been looked at; Place History, the visit editor and Add Visit have not. `xcrun simctl ui <udid> content_size accessibility-extra-extra-extra-large` plus `appearance dark`, then launch with `-ui-test-seed`, is the loop.
...
- [ ] Expand movement classification for walking, running, cycling and possible flights while preserving the location-first rule: movement inside Home, Work or another destination must not become a separate timeline entry. Automotive travel is classified and its short classifier dropouts are rejoined above; whether Core Motion misses whole drives remains a device-validation question.
