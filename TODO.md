...
- [ ] **`ActivityLocationPolicy` (819 lines) still mixes six concerns: shared classification predicates, stay dedup/confidence scoring, journey/movement bounding, Timeline/Insights visibility rules, travel-destination labelling, and reconciliation orchestration.** `ActivityImportActor` had the same shape of problem — reading mixed with writing — and has been split (below) along a seam that already existed as two separate `actor` types in one file; `ActivityLocationPolicy` needs the same treatment but its six concerns are function groups within one type, not two types, so it is a bigger job: roughly five new files plus a slimmer policy surface, and ~107 external call sites across 16 files whose `ActivityLocationPolicy.*` qualifier would need to change to whichever new type now owns that function. Mechanical and compiler-verified, but not small.
...
- [ ] **`InferenceEngine` discards a correctly-identified `MKPointOfInterestCategory` by round-tripping it through prose.** `PlaceLookupService.mapsHint(from:)` already reads the real category and maps it to a coarse string (e.g. `.pharmacy` → `"Healthcare"`), which `InferenceEngine.activity(placeName:mapsHint:)` then re-parses with its own, differently-worded keyword list (`["hospital", "doctor", "medical", "dentist", "pharmacy"]`). The word `"healthcare"` contains none of those, so a pharmacy Apple correctly identified falls through to the generic `"Visiting"` default — not a coverage gap fixable by adding keywords, but two independent vocabularies describing the same signal that cannot help but drift apart. `SmartActivityClassifier` takes the same coarse string but hands it to an LLM, which tolerates prose fine; it only runs for the top suggestion, confidence ≥60, when Apple Intelligence is available. `SavedPlaceLearning.swift:142`, `LocationRecorder.swift:239` and `:529`, and `ManualVisitView.swift:122` all call `InferenceEngine.activity` directly, several with no `mapsHint` at all — the fragile keyword-matching path is the dominant one, not the exception.
