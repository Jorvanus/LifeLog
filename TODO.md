...
- [ ] **A repair marked done that never ran cannot retry.** Until 2026-08-08 the `@AppStorage` completion marker was written whether or not the work happened: the guard fell through when the timeline query was still empty at launch, which is far more often "not loaded yet" than "nothing to do". Fixed. Checked the other two markers rather than assuming: `automatic-location-deduplicated-v3` and `stay-splits-rejoined-v1` both call their work unconditionally and only make the *save* conditional, so their flags are set after something genuinely ran — the reconcile marker was the only one with an inner condition that could skip the work outright. The rule to keep is that a completion marker belongs on the path that did the work, never after a branch that might not have taken it.
- [x] **The Saved Place name field still writes the store on every keystroke.** `SavedPlaceEditor` (`PlacesView.swift:164`) bound `TextField("Name", text: $place.name)` straight to the model. Fixed 2026-08-08: `SavedPlaceEditor` now uses local `nameDraft` state initialized on appear, committing to `place.name` in `save()`, eliminating store mutations and query evaluation on every character typed.
- [ ] **`Diagnostics.performance` hides anything under 250 ms**, which is every repair on a store this size. That threshold is right for spotting slowness and useless for answering "did this run" — and it cost four builds on 2026-08-08 before the pass was made to say so unconditionally. Anything whose *absence* would be a bug needs a plain `Diagnostics.record`, not a timing sample. **All ten call sites audited 2026-08-08.** One real gap, now fixed: the background import in `ActivityDataService` recorded nothing unless it exceeded 250 ms, and it is the writer every visit correction is sequenced against — the two entries that explained the four failed fixes, at 334 ms and 361 ms, were luck. Three others (`RootView` launch setup, both `InsightsView` sites) were paired with a `Diagnostics.budget` call measuring the same interval unconditionally, so the `performance` sample only added a second, contradictory row — "Slow, 150 ms" beside "Budget pass, 150 ms / 250 ms"; those are removed. `TimelineView` was already covered by three unconditional records around its sample. The remaining five — trend history, activity statistics, place-history summary, journal import, HealthKit step query — are display computations whose results are on screen, so their absence is visible without a log and a timing sample is the right instrument. **The rule to keep:** `performance` answers "was this slow", `budget` and `record` answer "did this run", and only one of them should measure any given interval.
...
- [x] Add a location-event journal for personal diagnostics containing callback type, callback/arrival/departure times, coordinate, accuracy, distance from current visit, chosen resolution transition, and related visit identifier. Done 2026-08-08: `LocationEvent` (schema V5, a new table only), written by `LocationJournal` at five callback sites, read under Diagnostics → Location journal. Gated on the existing detailed-location switch because it holds precise coordinates, trimmed to 500, and clearable. **"Related visit identifier" is the visit's arrival time**, not a UUID — the store has never had one, `VisitCorrection` already correlates this way, and adding an identifier to `Visit` would have rewritten all 25,558 rows to migrate instead of adding an empty table.
