...
- [ ] **The Saved Place name field still writes the store on every keystroke.** `SavedPlaceEditor` (`PlacesView.swift:164`) binds `TextField("Name", text: $place.name)` straight to the model, which is the pattern the visit editor was moved off on 2026-08-08. It is much milder there — that editor holds no `@Query` and its body does no archive work — so the cost is only whatever the queries still mounted behind it re-run, and `PlacesView` holds one over every automatic and manual visit. Not measured, and never reported as slow. Worth either confirming it is fine or giving it the same draft-then-commit treatment; what earns it a line is that the visit editor's version was unusable on a full archive and the shape of it had gone unnoticed.
