...
5. Open Insights and measure Day, Week, Month, and Year once each. Change the date, return to Today, and repeat each window. Each window must remain within its documented bound.
6. Open Settings → Diagnostics and retain the exported report. Budget records contain elapsed milliseconds and aggregate item counts only.
7. Repeat steps 3–5 after a device restart. Compare the worst timing per operation rather than individual sample values.

## Diagnostic records

`Diagnostics` stores a budget event for launch and each Insights period fetch/snapshot rebuild. Each event includes the operation, elapsed milliseconds, budget, pass/over-budget status, and an aggregate item count. No precise location, place name, note, or HealthKit value is recorded.
